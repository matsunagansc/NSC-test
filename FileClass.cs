...
namespace Files{
    public class FilePath{  //
        ...
        public FilePath(){
            cp = Directory.GetCurrentDirectory()+@"\HelloWorld\";   //現在の階層を取得（表記は一つ上のフォルダまで）
            //cp = Environment.CurrentDirectory();  //プロセスの階層を取得.プロパティによって変更可能.
    ...
    public class FileWrite : FilePath{
        ...
        public void writedo(string? str){
            ...
            using(StreamWriter writer = new StreamWriter(cp+"write.txt", false, enc)){  //引数=>対象ファイル,追記:true/上書き:false,エンコード指定
                writer.WriteLine(str);
            }   //usingステートメントを使っているのでwriter.Closeしない（括弧を抜けた時点で破棄される）
        }
    }
    public class FileOut : FilePath{
        private static string path = @"C:\Users\NSC-0\gits\";    //ファイルの出力先パス
        public FileOut(){
        ...
        public void outdo(string str){
            ...
            Console.WriteLine(cp+str+" "+path+str);
